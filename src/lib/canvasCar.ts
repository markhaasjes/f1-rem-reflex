...
function roundRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, r: number, fill: string, stroke?: string, strokeWidth?: number) {
  ctx.beginPath()
...
export function drawF1Car(
  ...
  const [px, py] = projection.toScreen(dataX, dataY)
