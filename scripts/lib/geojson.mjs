...
// Centripetal Catmull-Rom (alpha = 0.5). The uniform variant overshoots and
// forms cusps/loops where hand-digitized control points sit unevenly around
// tight corners - which rendered as visible knots in the track edge at
// Kumhobocht and Arie Luyendijk. Centripetal parameterization is loop-free
// by construction (Yuksel et al.).
function catmullRomPoint(p0, p1, p2, p3, t) {
  const alpha = 0.5;
  const knot = (a, b, prev) => prev + Math.hypot(b.x - a.x, b.y - a.y) ** alpha || prev + 1e-6;
  const t0 = 0;
  ...
  const lerp = (a, b, ta, tb) => {
    ...
    const k1 = (tb - u) / w;
    const k2 = (u - ta) / w;
    return { x: k1 * a.x + k2 * b.x, y: k1 * a.y + k2 * b.y };
  };
...
// dense curve via centripetal Catmull-Rom interpolation between each control
