...
async function fetchDriverLap(driver) {
  ...
  const lapEndDistanceM =
    samples[Math.min(samples.length - 1, Math.round((LEAD_PAD_S + driver.lapDurationS) * SAMPLE_RATE_HZ))].distanceM;
...
function findCornerCandidates(samples) {
  ...
  const turnRate = samples.map((_, i) =>
    Math.abs(
      angleDiffDeg(
        headingAt(samples, Math.min(i + span, samples.length - 1), span),
        headingAt(samples, Math.max(i - span, 0), span),
      ),
    ),
  );
  ...
  for (let i = 1; i < samples.length - 1; i++) {
    if (samples[i].speedKph <= samples[i - 1].speedKph && samples[i].speedKph <= samples[i + 1].speedKph)
      indices.add(i);
  }
...
function buildGrandstands(referenceLap, corners) {
  ...
  const centroid = samples.reduce((acc, s) => ({ x: acc.x + s.x / samples.length, y: acc.y + s.y / samples.length }), {
    x: 0,
    y: 0,
  });
  ...
  function standCluster(centerX, centerY, sampleIndex, count) {
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        x: round(x + (dx / len) * GRANDSTAND_OFFSET_M, 2),
        y: round(y + (dy / len) * GRANDSTAND_OFFSET_M, 2),
...
function buildCircuit(referenceLap, officialOutline, officialDense) {
  ...
  const corners = CORNER_DEFINITIONS.map((def) => {
    ...
    return {
      ...
      x: round(sample.x, 2),
      y: round(sample.y, 2),
      distanceM: round(sample.distanceM, 2),
    };
  });
  ...
  corners.forEach((corner, i) => {
    ...
    const gapBeforeM = prev
      ? corner.distanceM - prev.distanceM
      : corner.distanceM - (corners.at(-1).distanceM - referenceLap.lapLengthM);
    const gapAfterM = next
      ? next.distanceM - corner.distanceM
      : corners[0].distanceM + referenceLap.lapLengthM - corner.distanceM;
    const roadPath = sliceCurveAroundPoint(
      officialDense,
      corner.x,
      corner.y,
      Math.min(MAX_LOOKBACK_M, gapBeforeM * 0.6),
      Math.min(MAX_LOOKAHEAD_M, gapAfterM * 0.6),
    );
...
function extractCornerForDriver(driverLap, corner, neighborGapsM) {
  const expectedDistanceM =
    driverLap.lapStartDistanceM + (corner.distanceM - (driverLap.referenceLapStartDistanceM ?? corner.distanceM));
  ...
  while (
    endIndex < driverLap.samples.length - 1 &&
    driverLap.samples[endIndex].distanceM - apexDistanceM < lookaheadCapM
  )
    endIndex++;
  ...
  const actionSample =
    brakeSample ??
    (liftSample && {
      t: round(liftSample.t - t0, 3),
      distanceM: round(liftSample.distanceM - distance0, 2),
      speedKph: liftSample.speedKph,
    });
  let actionType = 'none';
  ...
  return {
    ...
    samples,
    actionType,
    brakePoint: actionSample
      ? { t: actionSample.t, distanceM: actionSample.distanceM, speedKph: actionSample.speedKph }
      : null,
...
async function main() {
  ...
  for (const driver of DRIVERS) {
    ...
    console.log(
      `  ${driver.acronym}: ${driverLaps[driver.acronym].samples.length} samples, lap length ${driverLaps[driver.acronym].lapLengthM.toFixed(0)}m`,
    );
  }
  ...
  const oneLapPoints = referenceLap.samples
    .filter(
      (s) =>
        s.distanceM >= referenceLap.lapStartDistanceM &&
        s.distanceM < referenceLap.lapStartDistanceM + referenceLap.lapLengthM,
    )
    .map((s) => ({ x: s.x, y: s.y }));
  ...
  console.log(
    `  fit: rotation=${rotationDeg.toFixed(1)}deg scale=${scale.toFixed(3)} avgError=${transform.avgErrorM.toFixed(1)}m`,
  );
  ...
  const officialCentroid = officialForFit.reduce(
    (acc, p) => ({ x: acc.x + p.x / officialForFit.length, y: acc.y + p.y / officialForFit.length }),
    { x: 0, y: 0 },
  );
  const approxCorner1 = nearestSampleByDistance(
    referenceLap.samples,
    referenceLap.lapStartDistanceM + CORNER_DEFINITIONS[0].expectedIntoLapM,
  );
  ...
  for (const driver of DRIVERS) {
    ...
    circuit.corners.forEach((corner, i) => {
      ...
      const gapBefore = prev
        ? corner.distanceM - prev.distanceM
        : corner.distanceM - (circuit.corners.at(-1).distanceM - referenceLap.lapLengthM);
      const gapAfter = next
        ? next.distanceM - corner.distanceM
        : circuit.corners[0].distanceM + referenceLap.lapLengthM - corner.distanceM;
    ...
    await writeFile(
      new URL(`../src/data/drivers/${driver.acronym}.json`, import.meta.url),
